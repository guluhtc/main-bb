    ...
    <header className="flex flex-col xs:flex-row justify-between items-center w-full mt-3 border-b pb-7 sm:px-4 px-2 border-gray-500 gap-2">
      <Link href="/dream" className="flex space-x-2">
        <Image
          ...
          height={24}
        />
        <h1 className="sm:text-3xl text-xl font-bold ml-2 tracking-tight hidden md:block">
          DecorAI.xyz
        </h1>
      </Link>
      {email ? (
        <div className="flex items-center space-x-4">
          ...
          <button className="border-r border-gray-300 pr-4 flex space-x-2 hover:text-blue-400 transition" onClick={() => { signOut() }}>Logout</button>
          {photo ? (
            <Image
              alt="Profile picture"
              src={photo}
